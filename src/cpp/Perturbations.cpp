...
Vector Perturbations::set_ic(const double x, const double k) const
{
  ...
  double Omega_nu0 = cosmo->get_OmegaNu(0.0);
  double Omega_gamma0 = cosmo->get_OmegaGamma(0.0);
...
double Perturbations::get_tight_coupling_time(const double k) const
{
  ...
  for (int i = 0; i < n_x; i++)
  {
    ...
    double dtdtau = rec->dtaudx_of_x(x_array[i]);
    double Hp = cosmo->Hp_of_x(x_array[i]);
    if (dtdtau < std::min(10.0, 10.0 * c * k / Hp))
    {
...
int Perturbations::rhs_tight_coupling_ode(double x, double k, const double *y, double *dydx)
{
  ...
  double Omega_gamma0 = cosmo->get_OmegaGamma(0.0);
  double Omega_nu0 = cosmo->get_OmegaNu(0.0);
  double Omega_CDM0 = cosmo->get_OmegaCDM(0.0);
  double Omega_b0 = cosmo->get_OmegaB(0.0);
  double Hp = cosmo->Hp_of_x(x);
