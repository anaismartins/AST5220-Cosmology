...
namespace Utils
{
      ...
      double epsilon)
  {
    ...
    if (y_low * y_high >= 0.0)
    {
      ...
      for (int i = 0; i < nsearch; i++)
      {
        ...
        if (i == nsearch - 1)
          throw "Error binary_search_for_value. Could not find a good interval to start search\n";
      }
    }
    ...
    while (x_high - x_low > delta_x)
    {
      ...
      count++;
      if (count > nmax_iter)
        throw "Error binary_search_for_value. Value not found after nmax iterations\n";
    }
  ...
  double j_ell(const int ell, const double x)
  {
    ...
    if (ell >= 10 && x < (1.0 - 2.6 / sqrt(ell)) * ell)
      return 0.0;
...
{
  std::vector<double> y(x.size());
  std::transform(x.begin(), x.end(), y.begin(), [=](double t)
                 { return pow(t, n); });
  return y;
}
